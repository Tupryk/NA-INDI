...
def exportNet(pth_path: str,
              X_mins: np.ndarray=np.array([]),
              X_maxs: np.ndarray=np.array([]),
              y_mins: np.ndarray=np.array([]),
              y_maxs: np.ndarray=np.array([]),
              info: str=""):
    ...
    if len(X_mins):
        result += f"    float scaled_input[{INPUT_SIZE}];\n"
        result += f"    float X_mins[{INPUT_SIZE}];\n"
        result += f"    float X_maxs[{INPUT_SIZE}];\n"
    if len(y_mins):
        result += f"    float scaled_output[{OUTPUT_SIZE}];\n"
        result += f"    float y_mins[{OUTPUT_SIZE}];\n"
        result += f"    float y_maxs[{OUTPUT_SIZE}];\n"
    ...
    if len(X_mins):
        result += f"    .scaled_input = {arr2cstr(np.zeros(INPUT_SIZE))},\n"
        result += f"    .X_mins = {arr2cstr(X_mins)},\n"
        result += f"    .X_maxs = {arr2cstr(X_maxs)},\n"
    if len(y_mins):
        result += f"    .scaled_output = {arr2cstr(np.zeros(OUTPUT_SIZE))},\n"
        result += f"    .y_mins = {arr2cstr(y_mins)},\n"
        result += f"    .y_maxs = {arr2cstr(y_maxs)},\n"
    ...
    if len(X_mins):
        ...
        result += f"    for (int i = 0; i < INPUT_SIZE; i++) {{\n"
        result += f"        {exportname}.scaled_input[i] = "
        result += f"(((input[i]-{exportname}.X_mins[i])/({exportname}.X_maxs[i]-{exportname}.X_mins[i]))-0.5f)*2.f;"
        result += f"\n    }}\n"
    ...
    for key, data in state_dict.items():
        switcher += 1
        counter += 1
        input = "mlp.scaled_input" if counter == 1 else f"{exportname}.{name.replace('bias', 'activation')}"
        name = key.replace(".", "_")
        ...
        if switcher == 1:
            ...
            if len(list(state_dict.items())) != counter:
                activation_func = 2
            else:
                activation_func = 0
            result += f"{exportname}.{name}, {exportname}.{output}, {activation_func});\n"
            switcher = 0
    
    # Rescale outputs
    if len(y_mins):
        result += "    // Output normalization\n"
        result += f"    for (int i = 0; i < OUTPUT_SIZE; i++) {{\n"
        result += f"        {exportname}.scaled_output[i] = "
        result += f"({exportname}.{output}[i] + 1.f) * 0.5f * ({exportname}.y_maxs[i] - {exportname}.y_mins[i]) + {exportname}.y_mins[i];"
        result += f"\n    }}\n"
        result += "    return " + exportname + ".scaled_output;\n};\n"
    else:
...
def nn_c_model_test(original_model,
                    X_mins: np.ndarray=np.array([]),
                    X_maxs: np.ndarray=np.array([]),
                    y_mins: np.ndarray=np.array([]),
                    y_maxs: np.ndarray=np.array([]),
    ...
    if not len(test_data):
        test_data = np.random.random((1000, in_dim))
    
    gen_file = open(f'{PATH}test_data.h', 'w')
    gen_file.write(f"float input[{test_data.shape[0]}][{test_data.shape[1]}] = {arr2cstr(test_data[:100])};")
    ...
    test_data_ = (((test_data-X_mins)/(X_maxs-X_mins))-.5)*2
    tensor_input = torch.from_numpy(test_data_)
    py_output = original_model.forward(tensor_input).detach().numpy()
    if len(y_mins):
        py_output = (py_output + 1) * .5 * (y_maxs - y_mins) + y_mins
