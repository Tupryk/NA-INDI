...
def error_calculator(real_pos: List[float],
    ...
    # z = real_pos[2][cutoff_start:cutoff_end]
    x = real_pos[0][cutoff_start:cutoff_end]
    y = real_pos[1][cutoff_start:cutoff_end]
    z = real_pos[2][cutoff_start:cutoff_end]
    # target_pos[0] = target_pos[0][cutoff_start:cutoff_end]
    # target_pos[1] = target_pos[1][cutoff_start:cutoff_end]
    # target_pos[2] = target_pos[2][cutoff_start:cutoff_end]
    x_ = target_pos[0][cutoff_start:cutoff_end] - real_pos[0][cutoff_start:cutoff_end]
    y_ = target_pos[1][cutoff_start:cutoff_end] - real_pos[1][cutoff_start:cutoff_end]
    z_ = target_pos[2][cutoff_start:cutoff_end] - real_pos[2][cutoff_start:cutoff_end]
    # x_ = [target_pos[0][int(j/len(x)*len(target_pos[0]))]-x[j]
    #       for j, _, in enumerate(x)]
    # y_ = [target_pos[1][int(j/len(x)*len(target_pos[1]))]-y[j]
    #       for j, _, in enumerate(x)]
    # z_ = [target_pos[2][int(j/len(x)*len(target_pos[2]))]-z[j]
    #       for j, _, in enumerate(x)]

    vector = np.array([x_, y_, z_]).T
